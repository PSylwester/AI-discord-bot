...
# Komenda do tłumaczenia wiadomości na polski
...
# Komenda do tłumaczenia wiadomości na angielski
...
# Komenda do tłumaczenia wiadomości na niemiecki
...
# Komenda do tłumaczenia wiadomości na hiszpanski
...
async def translate_from_x_to_y(interaction: discord.Interaction,
    ...
    try:
        ...
        await interaction.response.send_message(f'**Translated from {language_map[src_language]} to {language_map[dest_language]}:** {translated_text}', ephemeral=True)
    except Exception as e:
        ...
        response = ollama.chat(model='llama3.2', messages=[
            {'role': 'system',
             'content': 'You are an assistant integrated with a Discord bot. '
                        ...
                        'For example:\n\n'
                        '"Translate klawiatura to English" => {"intent": "translate_to_english", "details": {"text": "klawiatura"}}\n'
                        '"Przetłumacz mi słowo komputer na angielski" => {"intent": "translate_to_english", "details": {"text": "komputer"}}\n'
                        '"Can you translate the word computer into German?" => {"intent": "translate_to_german", "details": {"text": "computer"}}\n\n'
                        'If the user specifies a target language that is not supported, '
                        'and clearly mentions the unsupported language, respond with {"intent": "unsupported_language", "details": {"language": "specified language", "text": "example text"}}.\n\n'
                        'If the user does not explicitly specify the target language in their message, respond with {"intent": "unknown"}. '
                        'For example:\n'
                        '"Przetłumacz mi tekst Today the weather is nice" => {"intent": "unknown"}\n\n'
                        'If the input contains only emoji, symbols, or non-meaningful content (e.g., "#$%^&", "😃🎉🌟"), respond with '
                        ...
                        'If the user requests to convert a text message to a voice message, respond with {"intent": "text_to_speech", "details": {"text": "example text"}}.\n\n'
                        'For example:\n'
                        '"Convert this message to voice: Hello world!" => {"intent": "text_to_speech", "details": {"text": "Hello world!"}}\n'
                        '"Zamień hello world na głosówkę" => {"intent": "text_to_speech", "details": {"text": "hello world"}}\n'
                        '"Zamień to na wiadomość głosową: Cześć!" => {"intent": "text_to_speech", "details": {"text": "Cześć!"}}\n\n'
...
async def text_to_speech_logic(message_content: str, interaction: discord.Interaction = None):
    try:
        ...
        tts = gTTS(text=message_content, lang=server_language)
        tts.save("message.mp3")

        if interaction:
            # Jeśli wywołane z interakcji, wyślij plik jako odpowiedź
